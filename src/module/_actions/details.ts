...
const fetchPaymentDetailAction = ({
    nextScreen,
    setPageLoading,
}: {
    ...
    const initiatePayment = async ({
        ...
        try {
            ...
            const response = (await client?.post(
                `${URL_PATHS.PAYMENT_INITIATE}/${fiatCurrency.toLowerCase()}`,
                {
    ...
    const RecordConfirmedTx = (response: AxiosResponse) => {
        ...
        if (payResponse?.data?.data) {
            const paymentResponse: IPaymentData = {
                ...
                excess: (
                    parseFloat(payResponse.data.data.amountReceived || "0") -
                    parseFloat(payResponse.data.data?.amount || "0")
                ).toFixed(4),
                coin: payResponse.data.data.currency?.name,
            };
            setPaymentDetails(
                (prev) => ({ ...prev, ...paymentResponse }) as IPaymentData
            );
            onEventResponse &&
                onEventResponse({
                    status: payResponse.data.data.status as PaymentStatusProps,
                    message: payResponse.data.message,
                    data: paymentResponse,
                });
            if (
                [PAYMENT_STATUS.OVER, PAYMENT_STATUS.COMPLETE].includes(
                    payResponse.data.data.status as any
                ) &&
                interval
            ) {
    ...
    const CallConfirmPayment = async (id: string, isBackground?: boolean) => {
        ...
        const addedHeaders = isBackground
            ? { "hide-notify": "true" }
            : undefined;
        try {
    ...
    const triggerPaymentConfirm = (reason?: "cancelled" | "expired") => {
        ...
        if (reason) {
            ...
            Logger.debug(
                isExpired ? "expiring-payment...." : "canceling-payment...."
            );
            confirmPaymentQueue.clearQueue();
